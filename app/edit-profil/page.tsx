...
// Optional: we keep the client helper, but we'll prefer server upload using CLOUDINARY_URL
...
const EditProfile = () => {
  ...
  const [photoWidgetUrl, setPhotoWidgetUrl] = useState<string | null>(null);
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Basic validations: size and type
  ...
  const onSubmit = async (data: EditProfileFormData) => {
    try {
      ...
      // Prefer URL from Cloudinary widget if available; otherwise keep existing foto
      let imageUrl = photoWidgetUrl || profile?.foto || '';
      // Optional fallback: if user selected file but didn't use the widget, you can still upload via server API
      if (!photoWidgetUrl && photo) {
        try {
          ...
          if (!res.ok) {
            ...
            throw new Error(`Upload API failed: ${text}`);
          }
          const json = await res.json();
          imageUrl = json.url || imageUrl;
        } catch (err) {
          console.error(err);
          toast.error('Gagal mengunggah foto ke Cloudinary. Coba tombol Upload via Cloudinary.');
                      ...
                      <div className="flex-1">
                        <div className="mt-2">
                          <CldUploadWidget
                            ...
                            options={{ multiple: false, resourceType: 'image', folder:'JobIn' }}
                            onSuccess={(result) => {
                              const info: any = (result as any)?.info || {};
                              const url = info.secure_url || info.url;
                              if (url) {
                                setPhotoWidgetUrl(url);
                                setPhoto(null);
                                setPhotoPreview(url);
                            ...
                            onError={(err) => {
                              console.error(err);
                              toast.error('Gagal membuka widget upload.');
                            }}
                          >
                            {({ open }) => (
